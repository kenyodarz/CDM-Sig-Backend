...
class AfpRestController {

    @Autowired
    var serviceAPI: AfpServiceAPI? = null
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
    fun find(@PathVariable id: String): Afp {
        return serviceAPI!!.getT(id)
    }

    @PostMapping("/save")
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
    fun save(@Valid @RequestBody entity: Afp, result: BindingResult): ResponseEntity<Any> {
    ...
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
    fun delete(@PathVariable id: String): ResponseEntity<Afp> {
        val entity: Afp? = serviceAPI!!.getT(id)
        if (entity != null) {
