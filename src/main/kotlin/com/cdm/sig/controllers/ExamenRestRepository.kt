...
class ExamenRestRepository {

    @Autowired
    var serviceAPI: ExamenServiceAPI? = null
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
    fun find(@PathVariable id: Long): Examen {
        return serviceAPI!!.getT(id)
    }

    @PostMapping("/save")
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
    fun save(@Valid @RequestBody entity: Examen, result: BindingResult): ResponseEntity<Any> {
    ...
    @PreAuthorize("hasRole('USER') or hasRole('MODERATOR') or hasRole('ADMIN')")
    fun delete(@PathVariable id: Long) : ResponseEntity<Examen> {
        val entity: Examen? = serviceAPI!!.getT(id)
        if(entity != null){
    ...
    fun findIncapacidadByEmpleado(@PathVariable id: String): ResponseEntity<Any>{
        val returnList: List<Examen> = serviceAPI!!.findExamenByEmpleado(id)
